...
peer2Name = input("Hello! Please enter the name of this contact:")
...
def sendC():
	...
	while True:
		...
		if (len(messageQueue) != 0):
			...
			socket1.sendto(messageToSend.encode(), ('127.0.0.1',12001))
			x1 = time.time()	
...
def listen():
	...
	while True:
		...
		receiveMessage=socket1.recvfrom(2048)
		...
		if(message == "ACK0" and sequenceNumber == 0):
			...
			ACK1 = 1
                        ...
                        print("\t\t\t" + peer2Name + ":" + message[1:])
                        expectedSequenceNumber = 1 - expectedSequenceNumber
                        socket1.sendto("ACK0".encode(),('127.0.0.1',12001))
		  
		elif(message[0] == "1" and expectedSequenceNumber == 1):
                        print("\t\t\t" + peer2Name + ":" + message[1:])
                        expectedSequenceNumber = 1 - expectedSequenceNumber
                        socket1.sendto("ACK1".encode(),('127.0.0.1',12001))
